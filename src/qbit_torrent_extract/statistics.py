...
class StatisticsManager:
    ...
    def start_extraction_run(self, directory: str, torrent_name: Optional[str] = None) -> str:
        ...
        with self.lock:
            ...
            self.logger.info(f"Started extraction run {run_id} for directory: {directory}")
            if torrent_name:
    ...
    def record_archive_processed(self, archive_path: Path, archive_type: str, 
                                success: bool, size_bytes: int = 0,
                                extracted_size_bytes: int = 0, 
                                compression_ratio: float = 0.0,
    ...
    def record_archive_skipped(self, archive_path: Path, reason: str = "already_processed"):
    ...
    def finish_extraction_run(self) -> Optional[ExtractionRunStats]:
        ...
        with self.lock:
            ...
            start_dt = datetime.fromisoformat(self.current_run.start_time.replace('Z', '+00:00'))
            ...
            if self.current_run.duration_seconds > 0:
                ...
                self.current_run.bytes_per_second = (
                    self.current_run.total_archive_size_bytes / self.current_run.duration_seconds
                )
            
            # Calculate average compression ratio
            if self.current_run.successful > 0:
                total_ratio = sum(
                    archive.compression_ratio for archive in self.current_run.archives 
                    if archive.success and archive.compression_ratio > 0
                )
                self.current_run.average_compression_ratio = total_ratio / self.current_run.successful
            ...
            self.logger.info(f"Completed extraction run {completed_run.run_id} - "
    ...
    def _save_run_statistics(self, run_stats: ExtractionRunStats):
        ...
        try:
            ...
            data["aggregated_stats"] = asdict(self._update_aggregated_stats(
                AggregatedStats(**data.get("aggregated_stats", {})), 
                run_stats
            ))
    ...
    def _update_aggregated_stats(self, aggregated: AggregatedStats, 
                               new_run: ExtractionRunStats) -> AggregatedStats:
        ...
        if aggregated.lifetime_archives_by_type:
            aggregated.most_common_archive_type = max(
                aggregated.lifetime_archives_by_type.items(), 
                key=lambda x: x[1]
            )[0]
        
        if aggregated.lifetime_errors_by_type:
            aggregated.most_common_error_type = max(
                aggregated.lifetime_errors_by_type.items(),
                key=lambda x: x[1]
            )[0]
        ...
        if new_run.duration_seconds > 0:
            if aggregated.fastest_extraction_seconds == float('inf'):
                ...
                aggregated.fastest_extraction_seconds = min(
                    aggregated.fastest_extraction_seconds, 
                    new_run.duration_seconds
                )
        
        # Update largest archive
        for archive in new_run.archives:
            aggregated.largest_archive_processed_bytes = max(
                aggregated.largest_archive_processed_bytes,
                archive.size_bytes
            )
    ...
    def get_recent_runs(self, limit: int = 10) -> List[ExtractionRunStats]:
        ...
        try:
            ...
            runs = [ExtractionRunStats(**run_data) for run_data in recent_runs_data[-limit:]]
    ...
    def export_statistics(self, export_path: Optional[Path] = None) -> Path:
        ...
        if export_path is None:
            export_path = Path.cwd() / f"extraction_stats_export_{int(time.time())}.json"
...
def categorize_error(error_message: str, exception_type: Optional[str] = None) -> ErrorType:
