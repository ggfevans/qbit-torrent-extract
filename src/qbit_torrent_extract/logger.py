    ...
    def get_torrent_logger(self, torrent_name: str) -> logging.Logger:
        ...
        with self.lock:
            if torrent_name not in self.torrent_loggers:
                self.torrent_loggers[torrent_name] = self._create_torrent_logger(
                    torrent_name
                )
...
class LoggingManager:
    ...
    def __init__(self, config: Optional[Config] = None):
        ...
        self.torrent_manager = (
            TorrentLogManager(self.config) if self.config.per_torrent_logs else None
        )
    ...
    def _setup_file_logging(self):
        ...
        self.main_logger.info(
            f"Log rotation: {self.config.log_rotation_size} bytes, {self.config.log_rotation_count} backups"
        )

    def get_logger(
        self, name: str, torrent_name: Optional[str] = None
    ) -> logging.Logger:
    ...
    def get_log_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "active_torrent_loggers": (
                len(self.torrent_manager.torrent_loggers) if self.torrent_manager else 0
            ),
        }
