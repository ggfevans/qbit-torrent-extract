...
class ArchiveExtractor:
    ...
    def extract_all(self, directory: str) -> Dict[str, any]:
        ...
        self.current_run_id = self.stats_manager.start_extraction_run(
            directory, self.torrent_name
        )
        ...
        for iteration in range(max_iterations):
            ...
            new_archives = [
                arch for arch in archives if arch not in self.extracted_archives
            ]
    ...
    def _extract_single_archive(self, archive_path: Path) -> bool:
        ...
        if not validation.is_valid:
            ...
            error_type = categorize_error(
                validation.error_message or "validation_failed"
            )
        ...
        if not within_limit:
            error_msg = (
                f"Archive exceeds maximum nested depth {self.config.max_nested_depth}"
            )
            log_with_context(
                logging.WARNING,
                error_msg,
        ...
        try:
            log_with_context(
                ...
                archive_path=archive_path,
                archive_type=validation.archive_type,
                extraction_ratio=validation.extraction_ratio,
            )
            ...
            self.stats_manager.record_archive_processed(
                archive_path=archive_path,
                archive_type=(
                    validation.archive_type if "validation" in locals() else "unknown"
                ),
                success=False,
                size_bytes=archive_size,
                extraction_time=extraction_time,
    ...
    def _extract_zip(self, archive_path: Path) -> bool:
        ...
        try:
            ...
            log_with_context(
                logging.ERROR,
                f"Permission denied extracting ZIP: {e}",
                torrent_name=self.torrent_name,
                archive_path=archive_path,
            )
            return False
        ...
        try:
            with rarfile.RarFile(archive_path, "r") as rf:
                if rf.needs_password():
                    log_with_context(
                        logging.WARNING,
                        "Skipping password-protected RAR",
                        torrent_name=self.torrent_name,
                        archive_path=archive_path,
                    )
                    return False
            ...
            log_with_context(
                logging.ERROR,
                f"Permission denied extracting RAR: {e}",
                torrent_name=self.torrent_name,
                archive_path=archive_path,
            )
            return False
        ...
        try:
            with py7zr.SevenZipFile(archive_path, "r") as szf:
                if szf.needs_password():
                    log_with_context(
                        logging.WARNING,
                        "Skipping password-protected 7z",
                        torrent_name=self.torrent_name,
                        archive_path=archive_path,
                    )
                    return False
            ...
            log_with_context(
                logging.ERROR,
                f"Permission denied extracting 7z: {e}",
                torrent_name=self.torrent_name,
                archive_path=archive_path,
            )
            return False
    ...
    def _extract_tar(self, archive_path: Path, archive_type: str) -> bool:
        ...
        try:
            ...
            log_with_context(
                logging.ERROR,
                f"Permission denied extracting TAR: {e}",
                torrent_name=self.torrent_name,
                archive_path=archive_path,
            )
            return False
