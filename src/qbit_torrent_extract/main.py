...
def main(directory: Path, preserve: bool, verbose: bool, config: str, 
         max_ratio: float, max_depth: int, log_dir: str, torrent_name: str,
         stats_file: str, show_stats: bool, export_stats: str) -> None:
    ...
    try:
        ...
        if show_stats or verbose:
            ...
            click.echo(f"Lifetime archives processed: {aggregated.lifetime_archives_processed}")
            click.echo(f"Success rate: {(aggregated.lifetime_successful / max(1, aggregated.lifetime_archives_processed)) * 100:.1f}%")
            ...
            if aggregated.average_archives_per_run > 0:
                click.echo(f"Average archives per run: {aggregated.average_archives_per_run:.1f}")
