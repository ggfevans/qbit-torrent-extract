...
class TestErrorCategorization:
    ...
    def test_validation_errors(self):
        ...
        assert categorize_error("Archive validation failed") == ErrorType.VALIDATION_ERROR
    ...
    def test_permission_errors(self):
        ...
        assert categorize_error("Insufficient permissions") == ErrorType.PERMISSION_ERROR
    ...
    def test_corruption_errors(self):
        ...
        assert categorize_error("Data corruption detected") == ErrorType.CORRUPTION_ERROR
    
    def test_zipbomb_errors(self):
        """Test zipbomb protection error categorization."""
        assert categorize_error("Extraction ratio exceeded") == ErrorType.ZIPBOMB_PROTECTION
        assert categorize_error("Potential zipbomb detected") == ErrorType.ZIPBOMB_PROTECTION
    ...
    def test_depth_errors(self):
        ...
        assert categorize_error("Maximum nested depth exceeded") == ErrorType.NESTED_DEPTH_EXCEEDED
        assert categorize_error("Archive depth limit reached") == ErrorType.NESTED_DEPTH_EXCEEDED
        assert categorize_error("Too many nested levels") == ErrorType.NESTED_DEPTH_EXCEEDED
    
    def test_extraction_errors(self):
        """Test extraction error categorization."""
        assert categorize_error("Failed to extract archive") == ErrorType.EXTRACTION_ERROR
        assert categorize_error("Extraction process failed") == ErrorType.EXTRACTION_ERROR
...
class TestStatisticsManager:
    ...
    def test_record_archive_skipped(self, stats_manager):
        ...
        stats_manager.record_archive_skipped(
            archive_path=Path("/test/existing.zip"),
...
class TestAggregatedStatsCalculations:
    ...
    def test_multiple_runs_aggregation(self, stats_manager):
        ...
        stats_manager.record_archive_processed(
            Path("/test/a2.rar"), "rar", False, 2000, extraction_time=0.5,
            error_type=ErrorType.CORRUPTION_ERROR
        )
        stats_manager.finish_extraction_run()
    ...
    def test_compression_ratio_averaging(self, stats_manager):
        ...
        stats_manager.record_archive_processed(
            Path("/test/a1.zip"), "zip", True, 1000, 
            extracted_size_bytes=3000, compression_ratio=3.0, extraction_time=1.0
        )
        stats_manager.record_archive_processed(
            Path("/test/a2.zip"), "zip", True, 1000,
            extracted_size_bytes=1000, compression_ratio=1.0, extraction_time=1.0
        )
